  ...
  return (
    <div role="group" aria-labelledby={`${id}-label`}>
      ...
      <div className="flex items-center gap-2">
        <input id={id} name={name} className="input w-28" type="number" step={step} min={min} max={max}
          aria-describedby={err ? `${id}-err` : `${id}-help`} aria-invalid={!!err}
          value={val} onChange={e=>setVal(parseFloat(e.target.value))} onBlur={()=>commit(val)} onKeyDown={(e)=>{ if(e.key==='Enter') commit(val); }} />
        <input className="w-full" type="range" min={min} max={max} step={step} value={val} aria-label={`${label} slider`} onChange={e=>commit(parseFloat(e.target.value))} />
