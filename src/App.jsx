  ...
  useEffect(() => {
    ...
    const step = (ts) => {
      ...
      try{
        ...
        console.error(err);
        setUi(u=>({...u, error: 'Simulation error. Please adjust parameters or reset.'}));
      }
    };
  ...
  return (
    <div className="h-screen flex flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        <Sidebar />
        <main className="flex-1 p-4 overflow-y-auto bg-bg-light dark:bg-bg-dark transition-colors" aria-labelledby="exp-title">
          <ErrorBoundary>
            <section className="card p-4 mb-4" role="region" aria-label="Experiment workspace">
              <div className="flex flex-wrap items-center justify-between mb-3 gap-2">
                ...
                <div className="space-x-2">
                  {!isRunning ? (
                    <button onClick={startSimulation} className="btn btn-primary" disabled={anyInvalid} aria-disabled={anyInvalid} aria-describedby={anyInvalid?'start-disabled':''}>
                      <i className="fas fa-play" aria-hidden="true"></i> Start
                    </button>
                  ) : (
                    <button onClick={pauseSimulation} className="btn btn-primary">
                      <i className="fas fa-pause" aria-hidden="true"></i> Pause
                    </button>
                  )}
                  <button onClick={resetSimulation} className="btn btn-ghost"><i className="fas fa-rotate-left" aria-hidden="true"></i> Reset</button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <form className="space-y-4" aria-label="Parameter controls">
                  ...
                  {selectedExperiment === 'fluid_gl' && (
                    <div className="space-y-2" role="group" aria-label="Obstacles">
                      ...
                      <div className="flex gap-2">
                        <button type="button" className="btn btn-ghost" onClick={()=>exp.addObstacleRect(0.45,0.2,0.55,0.8)}>Center Pillar</button>
                        <button type="button" className="btn btn-ghost" onClick={()=>exp.addObstacleRect(0.1,0.4,0.2,0.6)}>Left Block</button>
                        <button type="button" className="btn btn-ghost" onClick={()=>exp.addObstacleRect(0.8,0.3,0.9,0.7)}>Right Block</button>
                      </div>
                    </div>
                  )}
                </form>
